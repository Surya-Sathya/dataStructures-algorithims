...
#For 2^11, we get 4 print statements. Each print statement at worst represents two multiplications (for when x is odd)
#The amount of recursive calls we get is how many times we can split 11 into halves, or log2(11) ~ 3.459 = 4 calls
#Hence Hence the amount of time steps to mulitply t(n) = 2*log(n) + 2*log(n) (the amount of if statements in comparison)
# O(4log(n)) -> O(log(n))